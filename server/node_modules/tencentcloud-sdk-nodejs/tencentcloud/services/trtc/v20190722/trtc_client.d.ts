...
3、客户端混流和服务端混流不能混用。
     ...
     * 语音合成接口
     */
    TextToSpeech(req: TextToSpeechRequest, cb?: (error: string, rep: TextToSpeechResponse) => void): Promise<TextToSpeechResponse>;
    /**
...
方式二：通过订阅[监控仪表盘](https://cloud.tencent.com/document/product/647/81331)商业套餐包「基础版」（可查近7天）和「进阶版」（可查近14天），[前往订阅](https://buy.cloud.tencent.com/trtc_monitor)。
     ...
     * 声音克隆
     */
    VoiceClone(req: VoiceCloneRequest, cb?: (error: string, rep: VoiceCloneResponse) => void): Promise<VoiceCloneResponse>;
    /**
     * 获取TRTC录制的用量明细。
- 查询时间小于等于1天时，返回每5分钟粒度的数据；查询时间大于1天时，返回按天汇总的数据。
- 单次查询统计区间最多不能超过31天。
